 ...
 * N-1, signée), et NON `VARIATION_VALEUR_ABSOLUE` (|N − N-1|, que
 * `NoteAnnexeService` calcule par `Math.abs`) : arbitrage pris contre la
 * lecture littérale du libellé, parce que deux autres maquettes du ch. 6 le
 * tranchent. La NOTE 14 « Primes et réserves » n'a QUE les colonnes
 * « Année N · Année N-1 · Variation en valeur absolue », sans colonne
 * « en % » : « absolue » y oppose le MONTANT au TAUX, il n'y a rien d'autre
 * à quoi l'opposer. Et le premier tableau de la NOTE 12 met « Variation en
 * valeur absolue » en regard de « Cours UML Année acquisition » et « Cours
 * UML 31/12 » : une variation de cours prise en |.| perdrait le sens même
 * de l'écart de conversion, actif ou passif. Une dette qui BAISSE de 500
 * afficherait sinon +500 dans une colonne pendant que la colonne « en % »
 * afficherait -20 %. Même lecture que la première tranche (son anomalie
 * n° 5, notes 14, 15A et 15B) : les deux tranches sont alignées.
 *
 ...
 *    voir la section « Colonnes » ci-dessus pour les deux maquettes du ch. 6
 *    qui le tranchent (notes 12 et 14). Le libellé de la colonne reste celui
 *    du texte, mot pour mot ; seul son CALCUL s'écarte de sa lettre. Même
 *    arbitrage que la première tranche (son anomalie n° 5).
 *
