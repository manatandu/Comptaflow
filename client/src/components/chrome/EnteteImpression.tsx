  ...
  return (
    <header className="impression-seul mb-4 pb-2 border-b-2 border-black">
      <div className="flex items-start justify-between gap-6">
        <div>
          <div className="text-[14px] font-bold uppercase">{tenant?.nom}</div>
          <div className="text-[11px]">
            Référentiel {tenant?.referentiel}
            {tenant?.referentiel === 'SYCEBNL' && ` · ${jeu}`}
            {tenant?.referentiel === 'SYSCOHADA' && systeme && ` · ${systeme}`}
          </div>
          {tenant?.numeroImpot && <div className="text-[11px]">N° d'identification fiscale {tenant.numeroImpot}</div>}
          {tenant?.devise && <div className="text-[11px]">Montants exprimés en {tenant.devise}</div>}
        </div>
        <div className="text-right">
          <div className="text-[14px] font-bold">{titre}</div>
          {sousTitre && <div className="text-[12px]">{sousTitre}</div>}
          {clos && debut && (
            <div className="text-[11px]">
...
export function BoutonImprimer({ libelle = 'Imprimer' }: { libelle?: string }) {
  return (
    <button
      ...
      className="ecran-seul flex items-center gap-1.5 border border-border rounded-[6px] bg-surface px-3 py-1.5 text-[12px] font-bold hover:bg-surface-alt"
    >
...
export function BlocCertification() {
  ...
  return (
    <section className="impression-seul mt-6 border border-black p-3 text-[12px]">
      <div className="font-bold uppercase mb-3">Certifié sincère et conforme aux règles {regles}</div>
      <div className="grid grid-cols-3 gap-6">
        <div>
          <div className="text-[11px]">Nom</div>
          <div className="h-6 border-b border-black" />
        </div>
        <div>
          <div className="text-[11px]">Qualité</div>
          <div className="h-6 border-b border-black" />
        </div>
        <div>
          <div className="text-[11px]">Signature et date</div>
          <div className="h-6 border-b border-black" />
        </div>
      </div>
    </section>
