...
export function RapprochementDetailPage({ id: idProp }: { id?: string } = {}) {
  ...
  return (
    <div className="p-2">
      ...
      {!detail && !erreur && <div className="text-[12.5px] text-text-dim">Chargement…</div>}
      {erreur && <div className="text-[12.5px] text-danger bg-danger-soft border border-danger/30 px-3 py-2 mb-3 max-w-[900px]">{erreur}</div>}
        ...
        <>
          ...
          >
            ...
            {detail.lignes.map((l, i) => (
              <div
                key={l.id}
                className={`grid grid-cols-[26px_70px_46px_1.4fr_100px_100px] min-w-[570px] gap-2.5 px-3.5 py-1.5 items-center border-b border-border last:border-b-0 text-[12px] ${
            ...
            {detail.lignes.length === 0 && (
              <div className="p-3 text-[12.5px] text-text-dim">Aucun mouvement pointable sur ce compte.</div>
            )}
          </div>
