    ...
    <>
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          ...
          <Aide titre="Balances des entités" texte="La balance de la consolidante est celle du grand livre. Celle de chaque autre entité s’importe au canevas de la balance agrégée (Numéro, Intitulé, Débit, Crédit), en francs congolais, déjà retraitée aux règles du groupe (D4C, ch. XII-3). Un nouvel import remplace le précédent. Le tableau des flux consolidé demande en plus les MOUVEMENTS de l’exercice · importez alors la balance à six colonnes (report, mouvements et solde, chacun en débit et en crédit), les colonnes étant reconnues à leur en-tête. Report et mouvements qui ne donnent pas le solde font refuser la ligne." source="D4C, ch. XII-3" />
        </h2>
        {entites.map((e) => (
          <div key={e.id} className="flex flex-wrap items-center gap-2 py-1 border-b border-border/60 text-[11.5px]">
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          Acquisitions
          <Aide titre="Acquisitions" texte="Pour chaque participation retenue · le coût des titres, frais directs compris, le compte qui le porte chez la détentrice, la date d’entrée et les capitaux propres de la détenue à cette date, résultat compris (art. 82). L’écart est rapporté au résultat selon un plan d’amortissement (art. 82) · durée d’utilité, ou dix ans si elle n’est pas déterminable (ch. XII-6 § 4)." source="AUDCIF art. 82 · D4C ch. XII-6 § 4" />
        </h2>
        {liens.map((l) => (
          <details key={l.id} className="border-b border-border/60 py-1">
            ...
            >
              <label className="text-[11.5px]">Coût d’acquisition (FC)<input name="cout" className={champ} defaultValue={l.coutAcquisition ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Compte des titres<input name="compteTitres" className={champ} defaultValue={l.compteTitres ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Date d’entrée<input name="dateEntree" type="date" className={champ} defaultValue={l.dateEntree?.slice(0, 10) ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Capitaux propres à l’entrée (FC)<input name="cpEntree" className={champ} defaultValue={l.capitauxPropresEntree ?? ''} disabled={!peutEcrire} /></label>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          Comptes réciproques
          <Aide titre="Comptes réciproques" texte="Créances et dettes, charges et produits entre entités intégrées (art. 86, 6°), déclarés après confirmation de solde. Avec une entité intégrée proportionnellement, l’élimination est limitée à sa fraction (ch. XII-5 § 5)." source="AUDCIF art. 86, 6° · D4C ch. XII-5 § 5" />
        </h2>
        {peutEcrire && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 mb-2">
            ...
            <label className="text-[11.5px]">Compte de A<input className={champ} value={recip.compteA} onChange={(e) => setRecip({ ...recip, compteA: e.target.value })} /></label>
            <label className="text-[11.5px]">Montant confirmé (FC)<input className={champ} value={recip.montant} onChange={(e) => setRecip({ ...recip, montant: e.target.value })} /></label>
            <label className="text-[11.5px]">
              Entité B
            ...
            <label className="text-[11.5px]">Compte de B<input className={champ} value={recip.compteB} onChange={(e) => setRecip({ ...recip, compteB: e.target.value })} /></label>
            <label className="text-[11.5px]">Libellé<input className={champ} value={recip.libelle} onChange={(e) => setRecip({ ...recip, libelle: e.target.value })} /></label>
            <button
              className="border border-border px-2.5 py-1 text-[11.5px] justify-self-start"
              onClick={() =>
                void agir(async () => {
        ...
        {reciproques.length === 0 ? (
          <p className="text-[11.5px] text-text-dim">Aucune opération réciproque déclarée.</p>
        ) : (
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        ...
        {peutEcrire && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 mb-2">
            ...
            <label className="text-[11.5px]">Compte de l’acheteuse<input className={champ} value={interne.compteActif} onChange={(e) => setInterne({ ...interne, compteActif: e.target.value })} /></label>
            <label className="text-[11.5px]">Marge à l’ouverture (FC)<input className={champ} value={interne.margeOuverture} onChange={(e) => setInterne({ ...interne, margeOuverture: e.target.value })} /></label>
            <label className="text-[11.5px]">Marge à la clôture (FC)<input className={champ} value={interne.margeCloture} onChange={(e) => setInterne({ ...interne, margeCloture: e.target.value })} /></label>
            <label className="text-[11.5px]">Libellé<input className={champ} value={interne.libelle} onChange={(e) => setInterne({ ...interne, libelle: e.target.value })} /></label>
            <button
              className="border border-border px-2.5 py-1 text-[11.5px] justify-self-start"
              onClick={() =>
                void agir(async () => {
        ...
        {resultatsInternes.length === 0 ? (
          <p className="text-[11.5px] text-text-dim">Aucun résultat interne déclaré.</p>
        ) : (
          <table className="w-full text-[11.5px]">
            <tbody>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          Écarts d’évaluation
          <Aide titre="Écarts d’évaluation" texte="La part de l’écart de consolidation affectée à un élément identifiable de la détenue, à sa juste valeur à l’entrée (art. 82, D4C ch. XII-6) · une immobilisation, un stock ou un passif externe, jamais les capitaux propres. Il passe EN PRIORITÉ, et l’écart d’acquisition n’est que le reste. Montant positif si l’élément vaut plus au bilan consolidé qu’aux livres de la détenue. Chaque écart porte son impôt différé au taux déclaré de la détenue, et appartient aux majoritaires comme aux minoritaires. Aucun écart d’évaluation sur une entité mise en équivalence." source="AUDCIF art. 82 · D4C ch. XII-6" />
        </h2>
        {peutEcrire && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 mb-2">
            <label className="text-[11.5px]">
            ...
            <label className="text-[11.5px]">Compte de l’élément<input className={champ} value={ecartForm.compte} onChange={(e) => setEcartForm({ ...ecartForm, compte: e.target.value })} /></label>
            <label className="text-[11.5px]">Écart à l’entrée (FC)<input className={champ} value={ecartForm.montant} onChange={(e) => setEcartForm({ ...ecartForm, montant: e.target.value })} /></label>
            <label className="text-[11.5px]">
              Sort dans le temps
            ...
            {ecartForm.mode === 'AMORTISSABLE' && (
              <>
                <label className="text-[11.5px]">Durée restant à courir à l’entrée (années)<input className={champ} value={ecartForm.dureeAnnees} onChange={(e) => setEcartForm({ ...ecartForm, dureeAnnees: e.target.value })} /></label>
                <label className="text-[11.5px]">Compte d’amortissement (28)<input className={champ} value={ecartForm.compteAmortissement} onChange={(e) => setEcartForm({ ...ecartForm, compteAmortissement: e.target.value })} /></label>
              </>
            )}
            {ecartForm.mode === 'REALISE' && (
              <label className="text-[11.5px]">Date de réalisation<input type="date" className={champ} value={ecartForm.dateRealisation} onChange={(e) => setEcartForm({ ...ecartForm, dateRealisation: e.target.value })} /></label>
            )}
            <label className="text-[11.5px]">Libellé<input className={champ} value={ecartForm.libelle} onChange={(e) => setEcartForm({ ...ecartForm, libelle: e.target.value })} /></label>
            <button
              className="border border-border px-2.5 py-1 text-[11.5px] justify-self-start"
        ...
        {ecartsEvaluation.length === 0 ? (
          <p className="text-[11.5px] text-text-dim">Aucun écart d’évaluation déclaré.</p>
        ) : (
          <table className="w-full text-[11.5px]">
            <tbody>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          Fiscalité des entités · impôts différés
          <Aide titre="Fiscalité des entités · impôts différés" texte="Le taux est celui « en vigueur à la clôture » (D4C ch. XII-3 § 3), déclaré avec sa source · OmegaX n’en écrit aucun, une filiale étrangère n’ayant pas le taux de la mère. Les impôts différés des comptes individuels (décalages temporaires, déficits reportables, art. 92) se déclarent en montants d’impôt · leur base fiscale est dans la liasse de l’entité, pas dans sa balance. Zéro est une réponse, un champ vide n’en est pas une, et l’état consolidé reste non publiable tant qu’une entité n’a pas répondu. Un impôt différé actif demande d’écrire pourquoi son imputation est probable." source="D4C ch. XII-3 § 3 · AUDCIF art. 92" />
        </h2>
        {[{ id: null as string | null, nom: `${consolidante.nom} (consolidante)`, f: fiscaliteConsolidante }, ...entites.map((e) => ({ id: e.id as string | null, nom: e.nom, f: e as FiscaliteCumul }))].map((x) => (
          <details key={x.id ?? 'consolidante'} className="border-b border-border/60 py-1">
            ...
            >
              <label className="text-[11.5px]">Taux d’impôt (%)<input name="taux" className={champ} defaultValue={x.f?.tauxImpotDiffere ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px] sm:col-span-2">Source du taux<input name="source" className={champ} defaultValue={x.f?.sourceTauxImpot ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Impôt différé actif, ouverture<input name="idaOuv" className={champ} defaultValue={x.f?.idaOuverture ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Impôt différé actif, clôture<input name="idaClo" className={champ} defaultValue={x.f?.idaCloture ?? ''} disabled={!peutEcrire} /></label>
              <span />
              <label className="text-[11.5px]">Impôt différé passif, ouverture<input name="idpOuv" className={champ} defaultValue={x.f?.idpOuverture ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Impôt différé passif, clôture<input name="idpClo" className={champ} defaultValue={x.f?.idpCloture ?? ''} disabled={!peutEcrire} /></label>
              <span />
              <label className="text-[11.5px]">Écarts de conversion actif (478), N-1<input name="ecaN1" className={champ} defaultValue={x.f?.ecartConversionActifN1 ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Écarts de conversion passif (479), N-1<input name="ecpN1" className={champ} defaultValue={x.f?.ecartConversionPassifN1 ?? ''} disabled={!peutEcrire} /></label>
              <span />
              <label className="text-[11.5px] sm:col-span-3">Pourquoi l’impôt différé actif est probable<input name="justif" className={champ} defaultValue={x.f?.justificationIda ?? ''} disabled={!peutEcrire} /></label>
              {peutEcrire && (
                <button type="submit" className="border border-border px-2.5 py-1 text-[11.5px] sm:col-span-3 justify-self-start">
                  Enregistrer
                </button>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          Monnaie des entités · conversion
          <Aide titre="Monnaie des entités · conversion" texte="Chaque entité déclare la monnaie de sa balance importée, qui doit être sa monnaie fonctionnelle (celle des prix de vente, des coûts et du financement, D4C ch. XII-4 § 1). Dans la monnaie des états consolidés, rien n’est converti ; dans une autre, la méthode du cours de clôture s’applique (§ 3) · actifs et passifs au cours de clôture, charges et produits au cours moyen de l’exercice ou de clôture, capitaux propres au cours historique, déclarés ici en montant. Les cours sont le prix d’une unité de la monnaie de l’entité. Le cours d’entrée sert à convertir l’écart d’acquisition. Une comptabilité tenue dans une autre monnaie que la fonctionnelle se convertit d’abord par la méthode temporelle, avant l’import. Tant qu’une entité n’a pas déclaré sa monnaie, l’état consolidé n’est pas publiable." source="D4C ch. XII-4 § 1 et § 3" />
        </h2>
        {entites.map((e) => (
          <details key={e.id} className="border-b border-border/60 py-1">
            ...
            >
              <label className="text-[11.5px]">Monnaie de la balance (code ISO)<input name="monnaie" className={champ} defaultValue={e.monnaieBalance ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px] sm:col-span-2">Pourquoi c’est sa monnaie fonctionnelle<input name="justifMonnaie" className={champ} defaultValue={e.justificationMonnaie ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Cours de clôture<input name="coursClo" className={champ} defaultValue={e.coursCloture ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Cours des charges et produits<input name="coursPc" className={champ} defaultValue={e.coursProduitsCharges ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px]">Cours à la date d’entrée<input name="coursEntree" className={champ} defaultValue={e.coursEntree ?? ''} disabled={!peutEcrire} /></label>
              <label className="text-[11.5px] sm:col-span-2">Capitaux propres hors résultat, au cours historique (monnaie des états)<input name="cpHist" className={champ} defaultValue={e.capitauxPropresHistoriques ?? ''} disabled={!peutEcrire} /></label>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5 mb-2.5">
        <h2 className="text-[11.5px] font-bold mb-1.5 flex items-center gap-1.5">
          ...
          <Aide titre="Provisions pour pertes de change" texte="Les écarts de conversion des comptes individuels (478, 479) s’éliminent en consolidation · il faut pour cela les déclarer en N-1 dans la fiscalité de l’entité (ci-dessus), et déclarer ici la provision qui couvrait la perte latente, avec sa dotation et sa reprise de l’exercice. Elle se loge au 194, au 4991 ou au 4997 · OmegaX ne la devine pas dans la balance · le solde du compte ne dit pas quelle part couvre la perte latente. Sans déclaration N-1, les 478 et 479 restent en l’état." source="Consolidation SYSCOHADA · comptes 478, 479, 194, 4991, 4997" />
        </h2>
        {peutEcrire && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 mb-2">
            <label className="text-[11.5px]">
            ...
            <label className="text-[11.5px]">Compte de la provision<input className={champ} value={provForm.compteProvision} onChange={(e) => setProvForm({ ...provForm, compteProvision: e.target.value })} /></label>
            <label className="text-[11.5px]">Solde à la clôture<input className={champ} value={provForm.cloture} onChange={(e) => setProvForm({ ...provForm, cloture: e.target.value })} /></label>
            <label className="text-[11.5px]">Dotation de l’exercice<input className={champ} value={provForm.dotation} onChange={(e) => setProvForm({ ...provForm, dotation: e.target.value })} /></label>
            <label className="text-[11.5px]">Reprise de l’exercice<input className={champ} value={provForm.reprise} onChange={(e) => setProvForm({ ...provForm, reprise: e.target.value })} /></label>
            <button
              className="border border-border px-2.5 py-1 text-[11.5px] justify-self-start self-end"
              onClick={() =>
                void agir(async () => {
        ...
        {provisionsChange.length === 0 ? (
          <p className="text-[11.5px] text-text-dim">Aucune provision pour pertes de change déclarée.</p>
        ) : (
          <table className="w-full text-[11.5px]">
            <tbody>
      ...
      <section className="border border-border bg-surface px-3.5 py-2.5">
        <div className="flex items-center gap-2 mb-1.5">
          <h2 className="text-[11.5px] font-bold">Balance consolidée avant répartition</h2>
          <button className="border border-border px-2.5 py-1 text-[11.5px]" onClick={() => void calculer()}>
            Calculer
          </button>
        </div>
        ...
        {resultat && (
          <>
            <table className="text-[11.5px] mb-2">
              <tbody>
            ...
            {Math.abs(resultat.equilibre) > 0.005 && (
              <p className="text-[11.5px] text-danger mb-2">La balance consolidée n’est pas équilibrée (écart de {fc(resultat.equilibre)}).</p>
            )}
            {resultat.avertissements.map((a) => <p key={a} className="text-[11.5px] text-warning">{a}</p>)}
