...
export function EcheancierPage() {
  ...
  return (
    <div className="p-2.5">
      ...
      <div className="ecran-seul flex items-end justify-between mb-2.5 gap-3 flex-wrap max-w-[1100px]">
        <div>
          <div className="text-[10.5px] font-mono text-text-dim">ÉTAT</div>
          <h1 className="text-[15px] font-bold">Échéancier de trésorerie</h1>
      ...
      {!etat && !erreur && <div className="text-[12px] text-text-dim">Chargement…</div>}

      {etat && (
        <div className="max-w-[1100px]">
          <div className="flex items-center gap-2 mb-2.5 border border-border bg-surface px-3.5 py-2">
            <span className="text-[11px] text-text-dim">Trésorerie disponible à la date de référence</span>
              ...
              return (
                <div key={t.cle}>
                  <button
                    ...
                    disabled={vide}
                    className={`w-full text-left grid grid-cols-[1fr_130px_130px_130px_150px] gap-2 px-4 py-1.5 text-[12px] border-b border-border/50 ${
                    ...
                    <div className="bg-chrome-alt border-b border-border">
                      ...
                      {etat.details
                        ...
                        .map((d) => (
                          <div
                            key={d.ligneId}
                            className="grid grid-cols-[80px_1fr_120px_120px_130px] gap-2 px-6 py-[3px] text-[11.5px]"
                          >
                            <span className="font-mono text-[10.5px]">{jour(d.date)}</span>
                            <span className="truncate">
                              {d.tiers && <span className="font-semibold">{d.tiers} · </span>}
                              {d.libelle}
                            </span>
                            <span className="font-mono text-[10.5px] text-text-dim">{d.compteNumero}</span>
                            <span className="font-mono text-[10.5px] text-text-dim">{d.reference ?? ''}</span>
                            <span
