...
export function BailleursPage() {
  ...
  return (
    <div className="p-2">
      ...
      {!jeuProjet && (
        <p className="text-[10.5px] text-text-dim mb-2.5">
      ...
      {estAdmin && afficherFormulaire && (
        <form onSubmit={onCreer} className="bg-surface border border-border p-4 mb-4 max-w-[520px]">
          <div className="font-mono text-[10.5px] font-semibold text-text-dim mb-3">NOUVEAU BAILLEUR</div>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={envoi} className="bg-sel text-white text-[11px] font-semibold px-4 py-1.5 disabled:opacity-50">
      ...
      {erreur && !afficherFormulaire && (
        <div className="text-[11px] text-danger bg-danger-soft border border-danger/30 px-2.5 py-1.5 mb-3 max-w-[900px]">{erreur}</div>
      )}
      ...
      {bailleurs && (
        ...
        >
          ...
          {bailleurs.length === 0 && <div className="px-4 py-3 text-[11px] text-text-dim">Aucun bailleur créé.</div>}
          {bailleurs.map((b, i) => (
            <div
              key={b.id}
            ...
            >
              ...
              {estAdmin && (
                <button onClick={() => basculerActif(b)} className="text-[10px] text-sel hover:underline text-left">
      ...
      {comptes && bailleurs && bailleurs.length > 0 && (
        <>
          <h2 className="text-[12px] font-bold mb-1.5">Comptes rattachés (162-164 / 462-464)</h2>
            ...
            {comptesEligibles.map((c, i) => (
              ...
              >
                ...
                <select
                  ...
                  disabled={!estAdmin}
                  className="border border-border-dark px-2 py-1 text-[10.5px] disabled:opacity-60"
                >
