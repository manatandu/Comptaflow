...
export function TauxTvaPage() {
  ...
  return (
    <div className="p-2">
      ...
      {erreur && (
        <div className="text-[12.5px] text-danger bg-danger-soft border border-danger/30 px-3 py-1.5 mb-2">{erreur}</div>
      )}
      ...
      >
        ...
        {liste?.map((t) => (
          <div
            key={t.id}
            className={`grid grid-cols-[80px_1fr_78px_210px_210px_80px] min-w-[890px] gap-2.5 items-center px-3.5 py-[4px] border-b border-border/50 last:border-b-0 text-[12px] hover:bg-sel-soft ${
          ...
          <form onSubmit={onCreer} className="anim-modale w-full max-w-[480px] bg-surface border border-border-dark shadow-flottante max-h-[calc(100dvh-2rem)] overflow-y-auto">
            ...
            <div className="p-4">
              <div className="grid grid-cols-[150px_1fr] items-center gap-x-3 gap-y-2.5">
                ...
                <label className="text-[12.5px] text-right">Taux (%) :</label>
                ...
                <label className="text-[12.5px] text-right">Collectée (443) :</label>
                <select value={compteCollecteId} onChange={(e) => setCompteCollecteId(e.target.value)} className="border border-border-dark px-2.5 py-1.5 text-[12.5px]">
                ...
                <label className="text-[12.5px] text-right">Déductible (445) :</label>
                <select value={compteDeductibleId} onChange={(e) => setCompteDeductibleId(e.target.value)} className="border border-border-dark px-2.5 py-1.5 text-[12.5px]">
                  <option value="">Aucun</option>
                  {comptesClasse4.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.numero} · {c.intitule}
