    ...
    <div className="p-2">
      ...
      <div className="flex items-center justify-end mb-1.5">
        <div className="flex items-center gap-2.5">
          ...
          <Aide
            titre="Tenue du registre"
            texte="« Il est établi pour chaque entité à but non lucratif un registre des donateurs pour tous les dons, donations et legs reçus par l’entité. » Sa tenue et sa mise à jour sont pénalement sanctionnées (art. 24). Une ligne erronée s’annule avec motif et conserve son numéro · la numérotation doit rester continue."
            source="SYCEBNL, art. 17 et 24"
          />
          {exerciceCourant && (
      ...
      {onglet === 'registre' && (
        <>
            ...
            <div className="mb-2.5">
              {!formOuvert ? (
                <button
                  ...
                  className="bg-sel text-white text-[11.5px] font-semibold px-3 py-1.5"
                >
                ...
                <div className="border border-border bg-surface p-3">
                  <div className="text-[11px] font-bold text-text-dim mb-2 flex items-center gap-1.5">
                    ...
                    <Aide
                      ...
                      texte="Le numéro d’ordre est attribué automatiquement. Les mentions de l’article 17 laissées vides ne bloquent pas l’inscription : elles sont signalées dans l’onglet Conformité."
                      source="SYCEBNL, art. 17"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="border border-border bg-surface text-[11.5px] px-3 py-1.5"
                    >
                      Annuler
                    </button>
                  </div>
          ...
          <div className="border border-border bg-surface overflow-x-auto">
            ...
            {lignes?.length === 0 && (
              <div className="px-3 py-4 text-[11.5px] text-text-dim">Aucune libéralité inscrite sur cet exercice.</div>
            )}
  ...
  const blocComptes = (titre: string, comptes: typeof r.comptesLiberalite, attenue: boolean) => (
    ...
    >
      ...
      {comptes.map((c) => (
        <div
          key={c.numero}
          className={`grid grid-cols-[64px_1fr_92px_110px] min-w-[470px] gap-2 px-3 py-1 border-b border-border last:border-b-0 text-[11.5px] ${
  ...
  return (
    <div>
      ...
      <div className={`border px-3.5 py-2.5 mb-2 ${r.rapproche ? 'border-positive/40 bg-positive-soft' : 'border-danger/30 bg-danger-soft'}`}>
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[11.5px] font-bold">Rapprochement avec la comptabilité</span>
          <span className={`text-[11.5px] font-bold ${r.rapproche ? 'text-positive' : 'text-danger'}`}>
      ...
      <div className="text-[11px] text-text-dim italic mt-2.5 flex items-center gap-1.5">
        Ces constatations ne valent pas avis.
        <Aide
          titre="Avis sur la tenue du registre"
          texte="« S’il existe un auditeur, ce dernier soumet […] un rapport qui constate l’existence du registre des donateurs et donne son avis sur sa tenue conforme. S’il n’existe pas d’auditeur, une déclaration des dirigeants attestant de la tenue conforme du registre des donateurs est annexée audit rapport ou soumise à l’assemblée générale ou l’instance qui en tient lieu. »"
          source="SYCEBNL, art. 18"
        />
      </div>
    </div>
  );
